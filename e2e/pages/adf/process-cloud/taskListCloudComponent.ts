...
export class TaskListCloudComponent {
    ...
    getRow(taskName) {
        return this.dataTable.getRowElement(this.columns.name, taskName);
    }
    ...
    getIdCellValue(rowName) {
        const locator = new DataTableComponentPage().getCellByRowAndColumn(this.columns.name, rowName, column.id);
